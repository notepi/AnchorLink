...
> 本章节基于学术研究和业界实践验证当前阈值设置的合理性

### 13.1 总体评估

| 阈值 | 当前设置 | 业界标准 | 评估结果 |
|------|---------|---------|---------|
| 行业涨跌幅判断 | ±0.5% | 日内方向判断±0.5-1% | **A 合理** |
| 相对强度排名 | 前30% | 学术前10%，券商前30% | **A 合理** |
| 放量判断 | 1.5倍 | 最小阈值1.5x，理想2x | **B 偏保守但可用** |
| 资金流向判断 | 60%正向 | Level-2标准60% | **A+ 与业界一致** |
| 板块轮动差值 | ±0.5% | 轻度1-2%，池子对比适用±0.5% | **B 偏低但可用** |
| 异常背离判断 | 2%偏离 | 统计套利2σ标准 | **A 与学术一致** |

**结论**：4/6阈值完全合理，2/6偏保守但在特定场景下可用。
...
| 应用场景 | 业界阈值 | 说明 |
|---------|---------|------|
| 日内方向判断 | ±0.5%-1% | 区分震荡 vs 有方向 |
| 月度轮动策略 | ±3%-5% | 周期性调仓触发 |
| 季度轮动策略 | ±8%-12% | 大趋势判断 |

**A股特性**：A股日均波动率~1%-2%，±0.5%能区分"随机波动"与"有方向行情"。
...
**学术依据**：
- [Jegadeesh & Titman (1993)](https://doi.org/10.1111/j.1540-6261.1993.tb04702.x)：前10%动量组合年化超额~12%
- [Top1000Funds研究](https://top1000funds.com/)：前10%收益更高但波动大，前30%更稳定
- [Moskowitz & Grinblatt (1999)](https://www.jstor.org/stable/117432)：行业动量使用前30%

**券商实践**：中信证券、华泰证券多因子选股常用前30%。

**评估**：**合理**，平衡收益与稳定性，符合A股券商实践。
...
| 来源 | 阈值 | 含义 |
|------|------|------|
| StockCharts | 50% above average (1.5x) | 最小确认阈值 |
| Investopedia | 100% above average (2x) | 理想确认阈值 |
| TradingView | 150% of normal (1.5x) | 常用阈值 |

**当前方案合理性**：1.5x是业界公认的"最小确认阈值"，能捕捉温和放量。

**长期优化方向**：提升至2x可减少假信号，符合Investopedia理想标准。

**评估**：**偏保守但可用**，适合池子成员少的场景。

#### 资金流向判断 60%

**业界标准**：
- [格隆汇](https://www.gelonghui.com/p/653051)：大单净流入>60%视为主力看好
- [东方财富](https://www.eastmoney.com/)：Level-2资金流向分析使用相同标准
- 连续3日>60%视为持续性主力行为

**评估**：**完全合理**，与Level-2数据分析业界标准一致。
...
| 来源 | 阈值 | 应用场景 |
|------|------|---------|
| ETFdb | 1-2%（轻度轮动） | 全市场行业对比 |
| ETFdb | 3-5%（中度轮动） | 明显资金迁移 |
| AnchorLink | ±0.5% | 池子成员少（4-10只）对比 |

**当前方案合理性**：池子成员少时差值自然更小，±0.5%适合小范围池子对比。

**长期优化方向**：提升至±1%可减少轻微轮动信号，符合ETFdb轻度标准。
...
**学术依据**：
- [Gatev et al. (2006) Pairs Trading](https://www.mit.edu/people/gatev/)：2σ偏离触发交易
- 统计套利经典方法使用2σ作为异常阈值
- Bollinger Bands使用±2σ作为技术分析标准

**A股特性**：2%偏离≈2倍A股日均波动（~1%），符合统计显著性。
...
| 特性 | A股 | 美股 | 对阈值的影响 |
|------|-----|------|------------|
| 涨跌幅限制 | ±10%/±20% | 无限制 | 异常阈值需排除涨跌停 |
| T+1制度 | T+1 | T+0 | 信号时效性要求更高 |
| 日均波动率 | 1%-2% | 0.5%-1.5% | 阈值应略放宽 |
| 散户占比 | 高 | 低 | 资金流向更复杂 |
...
| 阈值 | 当前值 | 建议优化值 | 原因 |
|------|-------|-----------|------|
| 放量判断 | 1.5x | 2x | Investopedia理想标准，减少假信号 |
| 板块轮动差值 | ±0.5% | ±1% | ETFdb轻度标准，减少轻微轮动信号 |

**优化时机**：如果实际使用中发现假信号过多，可考虑调整。

**代码位置**：阈值定义在 `src/signal/rules.py`，可随时调整。