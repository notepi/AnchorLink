...
| Symbol | Name | Role | Relevance | Benchmark | Ranking |
|--------|------|------|-----------|-----------|---------|
| 688433.SH | 华曙高科 | direct_comparable | 0.90 | ✅ | ✅ |

**industry_chain**：

| Symbol | Name | Role | Relevance | Benchmark | Ranking |
|--------|------|------|-----------|-----------|---------|
| 600343.SH | 航天动力 | downstream_demand | 0.85 | ✅ | ✅ |
| 600879.SH | 航天电子 | upstream_supplier | 0.80 | ✅ | ✅ |
| 600118.SH | 中国卫星 | downstream_demand | 0.75 | ✅ | ✅ |
| 003009.SZ | 中天火箭 | downstream_demand | 0.70 | ❌ | ✅ |

**theme_pool**（均不参与 Benchmark）：

| Symbol | Name | Role | Relevance |
|--------|------|------|-----------|
| 688433.SH | 华曙高科 | theme_heat_proxy | 0.65 |
| 601698.SH | 中国卫通 | theme_heat_proxy | 0.60 |
| 300762.SZ | 上海瀚讯 | theme_heat_proxy | 0.55 |
| 688062.SH | 航天宏图 | theme_heat_proxy | 0.50 |
| 688568.SH | 中科星图 | theme_heat_proxy | 0.45 |

**trading_watchlist**（均不参与 Benchmark）：

| Symbol | Name | Role | Relevance |
|--------|------|------|-----------|
| 688102.SH | 斯瑞新材 | trading_signal | 0.80 |
| 603678.SH | 火炬电子 | trading_signal | 0.85 |
| 000733.SZ | 振华科技 | trading_signal | 0.87 |
| 603267.SH | 鸿远电子 | trading_signal | 0.90 |
| 002179.SZ | 中航光电 | trading_signal | 0.53 |
| 600151.SH | 航天工程 | inactive_watch | 0.20（已禁用） |

**双重身份**：华曙高科(688433.SH)同时在 direct_peers(direct_comparable) 和 theme_pool(theme_heat_proxy)。
...
> 以下 Mermaid 图帮助直观理解上述概念。

### 一张图看懂 AnchorLink 在做什么
...
核心意思：四类池子都要先有自己的状态，后面才谈得上相对位置和组间轮动。
...
核心规则：不当 Benchmark，不等于不重要。不参与 Benchmark，不等于不计算状态。
