...
1. 全程不修改旧页面一行代码，旧页面随时可用，零风险
2. 最终结果100%对齐 `test/mub.html` 设计、100%对齐旧页面数据
3. 每步都有可验证的交付物
...
1. 将 `mub.html` 内联 CSS 迁移到 `web/src/styles/history-v2.css`，加作用域隔离
2. 按 DOM 结构翻译成 React 组件：TopBar、TradingCard、HistoryMapping、TrendChart、StabilityPanel、SignalTimeline、TransitionHeatmap、PersonalityProfile、PathPattern
3. 示例文本、数字照搬到组件
4. 复用 `mub.html` 媒体查询，测试 375/768/1440px+

#### 第2.5天：字段映射表

逐行扫描 `mub.html` 显示内容，标上序号，对应 `dashboard_view.json` 数据路径和格式化要求。

#### 第3-5天：动态数据对接

1. 格式化工具函数（日期、百分比、千分位、正负号颜色判断）
2. 数据读取 hook：从 `dashboard_view.json` 读取
3. 模块逐个对接：

| 模块 | 数据来源 |
|------|----------|
| 顶部信息栏 | meta + filter |
| 操盘建议卡片 | operator.playbook |
| 历史映射模块 | summary + similarCases + windowStats |
| 趋势图表模块 | trends（ECharts） |
| 稳定性监控模块 | personality.stability |
| 信号时间线模块 | trends.signalTimeline |
| 状态迁移模块 | tableData.transitions |
| 性格档案模块 | personality |
| 偏好路径模块 | personality.pathPatterns |

#### 第6天：交互逻辑还原

折叠面板、时间线 hover、热力图 hover、信号车道切换、Tab 切换、筛选器、按钮 hover/active 状态。

#### 第7天：全量测试校准

视觉校准、数据校准（110字段逐个对比）、交互校准、兼容性测试（Chrome/Firefox/Safari + 移动端）、性能测试（<2s）。

### 交付物

| 交付物 | 路径 |
|--------|------|
| 新页面路由 | `web/src/app/(main)/history-v2/page.tsx` |
| 新页面组件 | `web/src/components/history-v2/*` |
| 新页面样式 | `web/src/styles/history-v2.css` |
| 字段映射表 | `docs/数据契约.md` |
