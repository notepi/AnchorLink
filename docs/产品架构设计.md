...
| 层 | 名称 | 职责 | 输入 | 输出 | 详细设计 |
|---|------|------|------|------|----------|
| 第 1 层 | Anchor | 定义分析中心 | 配置文件 | 锚定标的 symbol | 第 3 节 |
| 第 2 层 | Universe | 定义四类股票池 | 配置文件 | 四类池子成员 | 第 4 节 |
| 第 3 层 | Market | 提供事实数据 | Tushare API | 涨跌/成交/资金/估值 | 第 5 节 |
| 第 4 层 | Pool State | 计算池子强弱 | Market 数据 | 每个池子的中位数/广度/资金 | 第 6 节 |
| 第 5 层 | Anchor Position | 判断 Anchor 位置 | Pool State + Anchor 数据 | 相对强弱/排名 | 第 7 节 |
| 第 6 层 | Group Rotation | 比较池子间强弱 | 四类 Pool State | 最强池/最弱池/spread | 第 8 节 |
| 第 7 层 | Signals | 生成有证据标签 | Position + Rotation | 行业Beta/个股Alpha/轮动标签 | 第 9 节 |
| 第 8 层 | Output | 输出三件套 | 全链路结果 | snapshot.json / peer_matrix.csv / report.md | 第 10 节 |
...
第 4 层: 计算每个池子自身状态
    ↓
第 5 层: 判断 Anchor 相对位置
    ↓
第 6 层: 比较四池强弱
    ↓
第 7 层: 生成有证据标签
    ↓
第 8 层: 输出 JSON/CSV/MD
```
...
- 每层只做自己职责内的事，不越界
- Market 层不判断强弱，只提供事实
- Pool State 层不看 Anchor，只看池子自身
- Signals 层不编故事，只翻译指标为标签
- Output 层不新增判断，只格式化输出
...
| `symbol` | 股票代码。 |
| `role` | 它在这个池子里扮演什么角色。 |
| `relevance` | 业务关联度评分（0-1），人工设定。 |
| `weight` | 计算权重（0-1），人工设定，未来可用于加权计算。 |
| `reason` | 为什么它应该进入这个池子。 |
| `include_in_benchmark` | 是否参与池子均值/中位数。 |
| `include_in_ranking` | 是否参与排名。 |
| `include_in_report` | 是否展示在报告里。 |

**relevance 和 weight 的设定依据**：

这两个字段目前是人工设定，不是算法输出。设定时参考：

| 因素 | 影响 | 举例 |
| --- | --- | --- |
| 业务关联度 | 直接可比 → relevance 高 | 华曙高科 relevance=0.90（同增材制造） |
| 主业纯度 | 纯度不足 → 降低权重 | 航天动力 relevance=0.35, weight=0.2 |
| 风险因素 | ST/监管风险 → 限权重 | ST臻镭 weight=0.6（有风险但重要） |
| 市值/弹性 | 大盘股弹性弱 → 低权重 | 中航光电 weight=0.4（盘子大） |
| 相关系数 | 近期相关性高 → relevance 高 | 可定量参考近20日相关系数 |

数值范围：
- `relevance`: 0.20 ~ 1.00（0.2 以下基本无关，仅观察）
- `weight`: 0.10 ~ 1.00（1.0 为满权重，0.1 为极低权重）

当前状态：这两个字段仅用于展示，未参与核心计算。未来可扩展为加权均值、加权中位数。
...
> 本节整合各模块的数据来源、计算逻辑、存储位置和前端组件信息。

### 16.1 信号面板（Signal Panel）
