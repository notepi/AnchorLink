...
# 默认路径
DEFAULT_CONFIG_PATH = Path(__file__).parent.parent.parent / "config" / "stocks.yaml"
DEFAULT_DATA_PATH = STORAGE.normalized_dir / "market_data_normalized.parquet"
DEFAULT_RAW_DATA_PATH = STORAGE.get_raw_path("market_data.parquet")
DEFAULT_OUTPUT_PATH = STORAGE.get_processed_path("daily_metrics.parquet")
...
def analyze_anchor_symbol(
    ...
    """
    # 1. 加载配置
    config = shared_load_config(config_path)
    ...
    moneyflow_df = None
    if data_path is None:
        from src.price.data_product import load_price_inputs
    ...
    valuation_percentile = _compute_valuation_percentile(
        anchor_symbol=anchor_symbol,
        daily_basic_df=daily_basic_df,
        latest_date=latest_date,
        lookback_days=60,
    ...
    multi_metric_valuation = compute_multi_metric_valuation_label(
        pe_ttm=basic_fields.get('pe_ttm'),
        pb=basic_fields.get('pb'),
        ps_ttm=basic_fields.get('ps_ttm'),
        pe_sector_position=sector_valuation['pe_sector_position'],
        pb_sector_position=sector_valuation['pb_sector_position'],
        ps_sector_position=sector_valuation['ps_sector_position'],
        pe_percentile_label=valuation_percentile['pe_percentile_label'],
        pb_percentile_label=valuation_percentile['pb_percentile_label'],
        ps_percentile_label=valuation_percentile['ps_percentile_label'],
    )

    # 用多维度综合标签覆盖原有标签
    valuation_label = multi_metric_valuation['valuation_label']
    valuation_detail = multi_metric_valuation['valuation_detail']

    print(f"[INFO] 多维度估值: {valuation_label} ({valuation_detail})")
    ...
    result = {
        ...
        **flow_fields,
        ...
        # v3.1 新增：多维度估值
        'valuation_detail': multi_metric_valuation['valuation_detail'],
        'valuation_score': multi_metric_valuation['valuation_score'],
        'sector_pe_mean': sector_valuation['sector_pe_mean'],
        'sector_pb_mean': sector_valuation['sector_pb_mean'],
        'sector_ps_mean': sector_valuation['sector_ps_mean'],
        'pe_vs_sector': sector_valuation['pe_vs_sector'],
        'pb_vs_sector': sector_valuation['pb_vs_sector'],
        'ps_vs_sector': sector_valuation['ps_vs_sector'],
        'pe_sector_position': sector_valuation['pe_sector_position'],
        'pb_sector_position': sector_valuation['pb_sector_position'],
        'ps_sector_position': sector_valuation['ps_sector_position'],
        'pe_percentile_60d': valuation_percentile['pe_percentile_60d'],
        'pb_percentile_60d': valuation_percentile['pb_percentile_60d'],
        'ps_percentile_60d': valuation_percentile['ps_percentile_60d'],
        'pe_percentile_label': valuation_percentile['pe_percentile_label'],
        'pb_percentile_label': valuation_percentile['pb_percentile_label'],
        'ps_percentile_label': valuation_percentile['ps_percentile_label'],
    }
...
def _get_position_label(value: float, sector_mean: float) -> str:
    """判断相对板块的位置"""
    if sector_mean is None or sector_mean <= 0:
        return "无法判断"
    if value is None:
        return "无法判断"

    ratio = value / sector_mean
    if ratio > 1.3:    # 高于板块均值30%
        return "偏高"
    elif ratio < 0.7:  # 低于板块均值30%
        return "偏低"
    else:
        return "适中"


def _get_percentile_label(percentile: float) -> str:
    """将分位数值转为标签"""
...
def _compute_sector_valuation(
    ...
    输出:
        sector_pe_mean: 板块PE均值
        sector_pb_mean: 板块PB均值
        sector_ps_mean: 板块PS均值
        pe_vs_sector: PE相对板块偏差
        pb_vs_sector: PB相对板块偏差
        ps_vs_sector: PS相对板块偏差
        pe_sector_position: PE板块位置（偏高/适中/偏低）
        pb_sector_position: PB板块位置
        ps_sector_position: PS板块位置
    """
    empty = {
        'sector_pe_mean': None, 'sector_pb_mean': None, 'sector_ps_mean': None,
        'pe_vs_sector': None, 'pb_vs_sector': None, 'ps_vs_sector': None,
        'pe_sector_position': "板块数据不足", 'pb_sector_position': "板块数据不足", 'ps_sector_position': "板块数据不足",
    }
    ...
    target_date = pd.Timestamp(latest_date).strftime('%Y%m%d') if hasattr(latest_date, 'strftime') else str(latest_date).replace('-', '')[:8]

    # 获取当日数据
    day_df = df[df['trade_date_str'] == target_date]
    if day_df.empty:
        return empty

    # anchor 估值
    anchor_row = day_df[day_df['ts_code'] == anchor_symbol]
    if anchor_row.empty:
        return empty

    anchor_pe = float(anchor_row.iloc[0]['pe_ttm']) if pd.notna(anchor_row.iloc[0].get('pe_ttm')) else None
    anchor_pb = float(anchor_row.iloc[0]['pb']) if pd.notna(anchor_row.iloc[0].get('pb')) else None
    anchor_ps = float(anchor_row.iloc[0]['ps_ttm']) if pd.notna(anchor_row.iloc[0].get('ps_ttm')) else None

    # core_universe 估值（用于板块均值）
    core_pes = []
    core_pbs = []
    core_pss = []
    ...
    for code in core_codes:
        ...
        if row.empty:
            continue
        r = row.iloc[0]
        if pd.notna(r.get('pe_ttm')) and r['pe_ttm'] > 0:
            core_pes.append(float(r['pe_ttm']))
        if pd.notna(r.get('pb')) and r['pb'] > 0:
            core_pbs.append(float(r['pb']))
        if pd.notna(r.get('ps_ttm')) and r['ps_ttm'] > 0:
            core_pss.append(float(r['ps_ttm']))

    # 板块均值
    sector_pe_mean = sum(core_pes) / len(core_pes) if core_pes else None
    sector_pb_mean = sum(core_pbs) / len(core_pbs) if core_pbs else None
    sector_ps_mean = sum(core_pss) / len(core_pss) if core_pss else None

    # 相对偏差
    pe_vs_sector = (anchor_pe - sector_pe_mean) / sector_pe_mean if anchor_pe and sector_pe_mean else None
    pb_vs_sector = (anchor_pb - sector_pb_mean) / sector_pb_mean if anchor_pb and sector_pb_mean else None
    ps_vs_sector = (anchor_ps - sector_ps_mean) / sector_ps_mean if anchor_ps and sector_ps_mean else None

    # 板块位置标签
    pe_sector_position = _get_position_label(anchor_pe, sector_pe_mean)
    pb_sector_position = _get_position_label(anchor_pb, sector_pb_mean)
    ps_sector_position = _get_position_label(anchor_ps, sector_ps_mean)

    return {
        'sector_pe_mean': sector_pe_mean,
        'sector_pb_mean': sector_pb_mean,
        'sector_ps_mean': sector_ps_mean,
        'pe_vs_sector': pe_vs_sector,
        'pb_vs_sector': pb_vs_sector,
        'ps_vs_sector': ps_vs_sector,
        'pe_sector_position': pe_sector_position,
        'pb_sector_position': pb_sector_position,
        'ps_sector_position': ps_sector_position,
    }
...
def _compute_valuation_percentile(
    ...
    输出:
        pe_percentile_60d: PE在近60日分位(0-100)
        pb_percentile_60d: PB在近60日分位
        ps_percentile_60d: PS在近60日分位
        pe_percentile_label: 高位/中位偏上/中位/中位偏下/低位
        pb_percentile_label: 同上
        ps_percentile_label: 同上
    """
    empty = {
        'pe_percentile_60d': None, 'pb_percentile_60d': None, 'ps_percentile_60d': None,
        'pe_percentile_label': "历史数据不足", 'pb_percentile_label': "历史数据不足", 'ps_percentile_label': "历史数据不足",
    }
    ...
    # 截取近 N 天数据
    start_date = latest - pd.Timedelta(days=lookback_days * 1.5)  # 多取一些自然日确保交易日足够
    window_df = anchor_df[(anchor_df['trade_date'] >= start_date) & (anchor_df['trade_date'] <= latest)]

    if len(window_df) < 20:
        return empty

    # 当日估值
    latest_row = anchor_df[anchor_df['trade_date'] == latest]
    if latest_row.empty:
        return empty

    latest_pe = float(latest_row.iloc[0]['pe_ttm']) if pd.notna(latest_row.iloc[0].get('pe_ttm')) else None
    latest_pb = float(latest_row.iloc[0]['pb']) if pd.notna(latest_row.iloc[0].get('pb')) else None
    latest_ps = float(latest_row.iloc[0]['ps_ttm']) if pd.notna(latest_row.iloc[0].get('ps_ttm')) else None
    ...
    pe_percentile = calc_percentile(latest_pe, window_df['pe_ttm'])
    pb_percentile = calc_percentile(latest_pb, window_df['pb'])
    ps_percentile = calc_percentile(latest_ps, window_df['ps_ttm'])

    return {
        'pe_percentile_60d': pe_percentile,
        'pb_percentile_60d': pb_percentile,
        'ps_percentile_60d': ps_percentile,
        'pe_percentile_label': _get_percentile_label(pe_percentile),
        'pb_percentile_label': _get_percentile_label(pb_percentile),
        'ps_percentile_label': _get_percentile_label(ps_percentile),
    }


def compute_multi_metric_valuation_label(
    pe_ttm: Optional[float],
    pb: Optional[float],
    ps_ttm: Optional[float],
    pe_sector_position: str,
    pb_sector_position: str,
    ps_sector_position: str,
    pe_percentile_label: str,
    pb_percentile_label: str,
    ps_percentile_label: str,
) -> Dict[str, Any]:
    """
    多指标综合估值标签（v3.1）
    ...
    输出:
        valuation_score: 综合评分(-2到+2)
        valuation_detail: 估值细节描述
        valuation_label: 综合估值标签
    """
    # 单指标评分映射
    sector_score_map = {"偏高": 1, "适中": 0, "偏低": -1, "无法判断": 0, "板块数据不足": 0}
    percentile_score_map = {
        "高位": 1, "中位偏上": 0.5, "中位": 0, "中位偏下": -0.5, "低位": -1,
        "无法判断": 0, "历史数据不足": 0
    }

    # 计算各指标得分
    def score_metric(sector_pos, percentile_label, has_value):
        if not has_value:
            return 0
        s = sector_score_map.get(sector_pos, 0)
        p = percentile_score_map.get(percentile_label, 0)
        return s * 0.4 + p * 0.6

    pe_score = score_metric(pe_sector_position, pe_percentile_label, pe_ttm is not None and pe_ttm > 0)
    pb_score = score_metric(pb_sector_position, pb_percentile_label, pb is not None and pb > 0)
    ps_score = score_metric(ps_sector_position, ps_percentile_label, ps_ttm is not None and ps_ttm > 0)

    # 加权综合得分
    total_score = pe_score * 0.5 + pb_score * 0.3 + ps_score * 0.2

    # 生成描述
    detail_parts = []
    if pe_ttm is not None and pe_ttm > 0:
        detail_parts.append(f"PE板块{pe_sector_position}+历史{pe_percentile_label}")
    if pb is not None and pb > 0:
        detail_parts.append(f"PB板块{pb_sector_position}")
    if ps_ttm is not None and ps_ttm > 0:
        detail_parts.append(f"PS板块{ps_sector_position}")

    valuation_detail = "，".join(detail_parts) if detail_parts else "估值数据不足"

    # 最终标签
    if total_score >= 0.8:
        valuation_label = "估值偏高"
    elif total_score >= 0.3:
        valuation_label = "估值适中偏上"
    elif total_score >= -0.3:
        valuation_label = "估值适中"
    elif total_score >= -0.8:
        valuation_label = "估值适中偏下"
    else:
        valuation_label = "估值偏低"

    # PE 为负（亏损）特殊处理
    if pe_ttm is not None and pe_ttm <= 0:
        valuation_label = "亏损状态"
        valuation_detail = "PE为负，估值指标不适用"

    return {
        'valuation_score': round(total_score, 2),
...
def _load_daily_basic(anchor_symbol: str, trade_date, source_df: Optional[pd.DataFrame] = None) -> dict:
    """
    从 raw 层加载 daily_basic，提取当日 PE/PB/PS/市值字段。
    文件不存在或当日无数据时返回空字段（不影响主流程）。
    """
    path = STORAGE.raw_dir / "daily_basic.parquet"
    empty = {
    ...
    try:
        ...
        result = {
            ...
            'ps_ttm':         float(r['ps_ttm'])          if 'ps_ttm'         in r and pd.notna(r['ps_ttm'])         else None,
        ...
        print(f"[INFO] daily_basic: PE_TTM={result['pe_ttm']}, PB={result['pb']}, PS_TTM={result['ps_ttm']}, "
...
def _load_moneyflow(anchor_symbol: str, trade_date, source_df: Optional[pd.DataFrame] = None) -> dict:
    ...
    文件不存在或当日无数据时返回空字段（不影响主流程）。
    """
    path = STORAGE.raw_dir / "moneyflow.parquet"
    empty = {
        'net_mf_amount': None,
