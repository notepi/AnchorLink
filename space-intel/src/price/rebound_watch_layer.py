...
用法：
    python3 -m src.price.rebound_watch_layer
    python3 -m src.price.rebound_watch_layer --date 20260317
...
def load_scored_metrics(date_str: str = None) -> dict:
    """加载评分数据"""
    df = pd.read_parquet(STORAGE.analytics_dir / "scored_metrics.parquet")
