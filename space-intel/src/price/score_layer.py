...
用法：
    python3 -m src.price.score_layer
    python3 -m src.price.score_layer --validate  # 验证历史表现
    python3 -m src.price.score_layer --date 20260317
...
def main():
    ...
    df.to_parquet(STORAGE.analytics_dir / "scored_metrics.parquet", index=False)
    print(f"[INFO] 评分结果已保存: {STORAGE.analytics_dir / 'scored_metrics.parquet'}")
    ...
    if args.validate:
        # 需要加载带收益的验证数据
        try:
            validation_df = pd.read_parquet(STORAGE.analytics_dir / "signal_validation_data.parquet")
            # 合并评分
            ...
            if not result.empty:
                ...
                # 保存验证结果
                result.to_parquet(STORAGE.analytics_dir / "score_validation.parquet", index=False)
                print(f"\n[INFO] 验证结果已保存: {STORAGE.analytics_dir / 'score_validation.parquet'}")
            else:
                print("[WARN] 验证样本不足")
