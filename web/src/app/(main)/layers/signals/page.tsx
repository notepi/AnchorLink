...
const SIGNAL_RULES = {
  ...
  alpha: [
    ...
    { label: '跑赢本业池', formula: 'position="outperform" AND relative_strength > 0.5%', threshold: '0.5%' },
    { label: '跑输本业池', formula: 'position="underperform" AND relative_strength < -0.5%', threshold: '-0.5%' },
  ...
  rotation: [
    { label: '本业池强于主题扩散', formula: 'core_vs_theme_spread > 1.0%', threshold: '1.0%' },
    { label: '主题扩散强于本业池', formula: 'core_vs_theme_spread < -1.0%', threshold: '-1.0%' },
    { label: '产业强于主题', formula: 'core_vs_trading_spread > 1.0%', threshold: '1.0%' },
    { label: '主题强于产业', formula: 'core_vs_trading_spread < -1.0%', threshold: '-1.0%' },
    { label: '交易联动池升温', formula: 'trading_pool.median_return > 0.5%', threshold: '0.5%' },
    { label: '交易联动池降温', formula: 'trading_pool.median_return < -0.5%', threshold: '-0.5%' },
  ],
  abnormal: [
    ...
    { label: '核心强但锚定标的弱', formula: '(未实现)', threshold: '--' },
          ...
          return (
            ...
            >
              <div className="flex items-center gap-3">
                <span className={`text-xs ${hit ? 'text-anchor-positive' : 'text-anchor-textMuted'}`}>
                  {hit ? '✓' : '✗'}
                </span>
                ...
                {signal && (
                  <div className="flex items-center gap-2">
                    ...
                    <span className={`px-1.5 py-0.5 rounded text-xs ${
                      signal.confidence === 'high' ? 'bg-anchor-positive/20 text-anchor-positive' :
        ...
        <div className="grid grid-cols-2 gap-4">
          <div className="bg-anchor-bg rounded-sm p-3">
            ...
            <div className="mt-2 space-y-1 text-xs">
              <div><span className="text-anchor-textMuted">margin_ratio ≥ 2.0</span> → <span className="text-anchor-positive">high</span></div>
          ...
          <div className="bg-anchor-bg rounded-sm p-3">
            ...
            <div className="mt-2 space-y-1 text-xs">
              <div><span className="text-anchor-textMuted">percentile ≤ threshold/2</span> → <span className="text-anchor-positive">high</span></div>
      ...
      <div className="bg-anchor-bgSecondary rounded-sm p-4 border border-anchor-border">
        ...
        <div className="bg-anchor-bg rounded-sm p-3 font-mono text-xs space-y-1">
          <div className="text-anchor-text">
            ...
            <span className="text-anchor-negative">返回空结果（不产生信号）</span>
          </div>
          <div className="text-anchor-text">
            <span className="text-anchor-textMuted">if</span> missing_data ≥ 3
            <span className="text-anchor-textMuted"> → </span>
            <span className="text-anchor-negative">insufficient_data</span>
          </div>
          <div className="text-anchor-text">
